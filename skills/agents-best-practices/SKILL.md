...
- design an operational UI/dashboard for agent work, including Kanban-style task launch/monitoring across projects;
...
For Kirill in Telegram, default to a concise first pass, especially for advice, scenario selection, cost-control, configuration, or “what should we do?” questions:

- Lead with the practical recommendation or top 3–5 options.
- Label `Факты`, `Гипотезы`, and `Рекомендация` when facts and strategy are mixed.
- Avoid full blueprint depth unless the user asks to “подробнее”, “распиши”, “сделай план”, or wants an implementation spec.
- Do not dump every checklist item from this skill into the first answer; use the checklist internally to select the most important points.

If the user asks for a full design or implementation blueprint, include:
